...
public class MineRotation : MonoBehaviour {
	...
	void Update () {
		if (RotationNumber > 0.0f && RotationNumber < 1.0f) {
			transform.Rotate (180f * Time.deltaTime, 180f * Time.deltaTime, 0f);
		}
		if (RotationNumber > 1.0f && RotationNumber < 2.0f) {
			transform.Rotate (-180f * Time.deltaTime, -180f * Time.deltaTime, 0f);
		}
		if (RotationNumber > 2.0f && RotationNumber < 3.0f) {
			transform.Rotate  (180f * Time.deltaTime,0f, 180f * Time.deltaTime);
		}
